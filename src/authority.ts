...
export class AuthorityEngine {
  evaluate(agent:Agent, capability:string, risk:"low"|"medium"|"high"):AuthorityDecision {
    ...
    if(risk==="high"||["funds.transfer","contract.sign","production.delete","person.terminate","secrets.rotate"].includes(capability)) return {allowed:true,requiresHuman:true,reason:"Consequential action crosses the human approval boundary"};
